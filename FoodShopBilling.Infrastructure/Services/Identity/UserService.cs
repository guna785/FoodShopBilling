...
namespace FoodShopBilling.Infrastructure.Services.Identity
{
	public class UserService : IUserService
	{
		...
		{
			_logger.LogInformation("Get All User Started");
			List<ApplicationUser> users = await _userManager.Users.ToListAsync();
			List<UserResponse> result = _mapper.Map<List<UserResponse>>(users);
			_logger.LogInformation("Get All users Response {response}", Newtonsoft.Json.JsonConvert.SerializeObject(result));
		...
		public async Task<IResult> RegisterAsync(RegisterRequest request, string origin)
		{
			...
			if (userWithSameEmail == null)
			{
				...
				{
					_logger.LogError("User Creation Error", string.Join(",", result.Errors.Select(a => $"{a.Code} : {a.Description}").ToList()));
					return await Result.FailAsync(result.Errors.Select(a => _localizer[a.Description].ToString()).ToList());
				}
			}
			else
			{
				_logger.LogError("Email {0} is already registered.", request.Email);
		...
		public async Task<DataTablesJsonResult> GetPaginatedAsync(IDataTablesRequest request)
		{
			...
			Expression<Func<ApplicationUser, UserResponse>> expression = e => new UserResponse
			{
				Id = e.Id,
				Name = e.Name,
			...
			UserFilterSpecification locationFilter = new(request.Search.Value);
