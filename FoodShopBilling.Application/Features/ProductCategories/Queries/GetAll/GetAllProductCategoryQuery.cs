...
namespace FoodShopBilling.Application.Features.ProductCategories.Queries.GetAll
{
    ...
    internal class GetAllProductCategoryQueryHandler : IRequestHandler<GetAllProductCategoryQuery, Result<List<ProductCategoryResponse>>>
    {
        ...
        public GetAllProductCategoryQueryHandler(IStringLocalizer<GetAllProductCategoryQueryHandler> localize,IAppCache cache, IUnitOfWork<int> unitOfWork, IMapper mapper, ILogger<GetAllProductCategoryQueryHandler> logger)
        {
            _localize = localize;
            _cache = cache;
